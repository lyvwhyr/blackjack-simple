...
// numb = ["A", "A", "A", "A", "A", "A", "A", "A", "A", "A", "A", "A", "A"];
let playerCard = [];
var playerCard = [];
var dealerCard = [];
...
//keeps the user from making outstanging bets limits the min and max bets
...
//loops through the cards and suits to give a value
for (s in suits) {
...
// zeros out players and dealers hand
function dealNew() {
    dValue.innerHTML = "?";
    playerCard = [];
    dealerCard = [];
...
//keeps from going over max number 0f cards
function redeal() {
    cardCount++;
    if (cardCount > 40) {
...
function deal() {
  // gives the player and dealer two cards
    for (x = 0; x < 2; x++) {        
        dealerCard.push(cards[cardCount]);
        dealerHolder.innerHTML += cardOutput(cardCount, x);
        if (x == 0) {
    // covers the dealer card
            dealerHolder.innerHTML += '<div id="cover" style="left:100px;"></div>';
        }
    // runs the card count to make sure  deck does not run out of cards  
        redeal();
        playerCard.push(cards[cardCount]);
        playerHolder.innerHTML += cardOutput(cardCount, x);
        redeal();
    }
    // checks total value if player gets blackjack end game
...
function cardAction(a) {
    ...
    switch (a) {
        case 'hit':
            ...
            break;
        case 'hold':
            playend(); //stops the anymore cards from being dealt
            break;
      // takes the bet value and doubles the bet value combination of hit and hold
        case 'double':
...
// shuffles array and returns in random order
function shuffleDeck(array) {
    for (var i = array.length - 1; i > 0; i--) {
        //turns any number into a whole number
