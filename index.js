...
function deal() {
  for (x = 0; x < 2; x++) {
    ...
    if (x == 0) {
      dealerHolder.innerHTML += '<div id="cover" style="left:600px;"></div>';
    }
    redeal();
...
//with position variable we can position the cards: 800, 800


function cardOutput(n, x) {
  var position = (x > 0) ? x * 60 + 600 : 600;
  return '<div class="icard ' + cards[n].icon + '" style="left:' +  position + 'px;">  <div class="top-card suit">' + cards[n].cardnum + '<br></div>  <div class="content-card suit"></div>  <div class="bottom-card suit">' + cards[n].cardnum +
    '<br></div> </div>';
}
...
function playend() {
  ...
  if ((playervalue < 22 && dealervalue < playervalue) || (dealervalue > 21 && playervalue < 22)) {
    message.innerHTML += '<span style="color:#DAA520;">You WIN! You won $' + betvalue + '</span>';
    mydollars = mydollars + (betvalue * 2);
  } else if (playervalue > 21) {
    message.innerHTML += '<span style="color:#FFD700;">Dealer Wins! You lost $' + betvalue + '</span>';
  } else if (playervalue == dealervalue) {
    message.innerHTML += '<span style="color:#FFD700;">PUSH</span>';
    mydollars = mydollars + betvalue;
  } else {
    message.innerHTML += '<span style="color:#FFD700;">Dealer Wins! You lost $' + betvalue + '</span>';
  }
